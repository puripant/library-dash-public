<script lang="ts">
	import type { Dataset, IAdd, TAdd, TDataset } from 'src/types';

	import saveItems from '../utils/saveItems';
	export let items: any, dataset: TDataset, add: TAdd;
	...
	function readJson(e) {
		...
		reader.onload = (e: ProgressEvent<FileReader>) => {
			const cards = JSON.parse(e.target.result as string) as IAdd[];
			console.log('cards', cards);
			for (const preset of cards) {
				add({
					...preset,
					dataset: dataset[capitalizeFirstLetter(preset.dataset as any)]
				});
			}
		};
	}
...
<div class="flex justify-evenly my-4">
	<label for="upload-json" class="cursor-pointer">
	...
	<input id="upload-json" class="hidden" type="file" accept=".json" on:input={readJson} />
